...
describe('Full Colored Svg Suite', () => {
  describe('Valid Svg (ColoredSvg component)', () => {
    beforeEach(() => {
      render(<Svg name={iconName} color={testColor} width={testWidth} height={testHeight} />);
    });
  ...
  describe('Valid SVG with default params', () => {
    beforeEach(() => {
      render(<Svg name={iconName} color={testColor} />);
    });
  ...
  describe('Invalid Svg', () => {
    it('should throw an error if an invalid color is provided', () => {
      expect(() => render(<Svg name={iconName} color="not-a-color" width={testWidth} height={testHeight} />)).toThrow(
        'Invalid color value: not-a-color'
      );
    });

    it('should throw an error if invalid width/height is provided', () => {
      expect(() => render(<Svg name={iconName} color={testColor} width="200pixels" height="200pixels" />)).toThrow(
        /Invalid size values: width=200pixels, height=200pixels/
      );
    });
  });
