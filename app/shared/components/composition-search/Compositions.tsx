'use client';
import { Autocomplete, AutocompleteRenderInputParams, debounce, styled } from '@mui/material';
...
export const movieOptions = [
  ...
  { title: 'It a Wonderful Life', year: 1946 },
...
export const Compositions = () => {
  const getOptionsAsync = (query: string): Promise<OptionType[]> => {
    return new Promise((resolve) => {
      setTimeout(() => {
        resolve(options.filter((option: OptionType) => option.title.toLowerCase().includes(query.toLowerCase())));
      }, 1500);
    });
  };
  ...
  //   return (
  //     <ListItem disableGutters key={option.title}>
