...
│ │ ├── route.ts # /api/events
│ │ └── [slug]/
│ │ └── route.ts # /api/events/:slug
├── middleware/ # Request handling middleware
│ ├── logger.ts # Logging
│ └── authentication.ts # Auth checks
├── lib/
│ └── axiosAPI.ts # Axios instance setup
├── constants/
├── middleware.ts # Root-level middleware (e.g. i18n, auth)
```
